	...
	function write_config($data) 
	{
		...
		if(is_writable($output_path)) {
			...
			if(fwrite($handle,$new)) {
				//Let us write the homepage
				//read the entire string
				$str=file_get_contents($route_path);

				$fp=fopen($route_path,'w+');

				$str = str_replace("install",'home',$str);

				// Chmod the file, in case the user forgot
				@chmod($route_path,0777);

				if(fwrite($fp,$str,strlen($str))) {
					$str=file_get_contents($autoload_path);

					$fp=fopen($autoload_path,'w+');

					$str = str_replace("'email'","'database', 'session'",$str);
